...
am4core.ready(function () {
  ...
  var currentIndex;
  var currentCountry = "Canada (Total)";

  // last date of the data
  var lastDate = new Date(
    canada_total_timeline[canada_total_timeline.length - 1].date
  );
  ...
  var countryIndexMap = {};
  var list = provinces_total_timeline[provinces_total_timeline.length - 1].list;
  ...
  function getSlideData(index) {
    if (index == undefined) {
      index = provinces_total_timeline.length - 1;
    }

    var data = provinces_total_timeline[index];
  ...
  mapChart.projection = new am4maps.projections.Stereographic();
  mapChart.deltaLongitude = 108;
  ...
  title.text = "COVID-19 Canada Spread Data";
  title.align = "left";
  ...
  slider.events.on("rangechanged", function (event) {
    var index = Math.round(
      (provinces_total_timeline.length - 1) * slider.start
    );
  ...
  sizeLabel.tooltipText =
    "Some provinces and territories have so many cases that bubbles for places with smaller values often look the same even if there is a significant difference between them. This slider can be used to increase maximum size of a bubble so that when you zoom in to a region with relatively small values you could compare them anyway.";
  sizeLabel.fill = am4core.color("#ffffff");
  ...
  filterLabel.tooltipText =
    "This filter allows to remove provinces and territories with many cases from the map so that it would be possible to compare states with smaller number of cases.";
  filterLabel.fill = am4core.color("#ffffff");
  ...
  lineChart.data = JSON.parse(JSON.stringify(canada_total_timeline));
  ...
  function setCountryData(countryIndex) {
    ...
    for (var i = 0; i < lineChart.data.length; i++) {
      var di = provinces_total_timeline[i].list;
  ...
  function showWorld() {
    ...
    for (var i = 0; i < lineChart.data.length; i++) {
      var di = canada_total_timeline[i];
      var dataContext = lineChart.data[i];
  ...
  function updateTotals(index) {
    if (!isNaN(index)) {
      var di = canada_total_timeline[index];
  ...
  function idToName(id) {
    return am4geodata_data_province_canada[id]
      ? am4geodata_data_province_canada[id]
      : id;
  }
});
